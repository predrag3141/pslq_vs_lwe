...
- _y<sub>ext</sub>_ not even being short! Though PSLQ is designed to produce short solutions, its intended use case is for non-integer inputs. In this scenario, any integer solution is (falsely) considered a win, and PSLQ terminates. There is a potential remedy to this problem, which -- for the most part -- is also outside the scope of this README. Suffice it to say that PSLQ makes a decision to swap a pair of rows of its internal matrix, H, in each iteration. The pair it chooses optimizes a function of the diagonal elements of H. But PSLQ also tracks an integer matrix, B, whose columns get close to the solution hyperplane. Rather than optimize the diagonal elements of H, the algorithm could be modified to optimize the size of the projection of B's columns onto that hyperplane. This would steadily sharpen the bound on the smallest solution of <_v_,_y<sub>ext</sub>_> = 0. Only when this bound can no longer be sharpened, would PSLQ be allowed to terminate. But the implementation of PSLQ that the code in this repository uses doesn't incorporate such a modification.
