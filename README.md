...
- A potential improvement in the solution size: The choice of rows to swap optimizes the diagonal of H as a proxy for a more difficult, but more revealing quantity to compute -- a certain minimum radius. This radius is that of the smallest projection onto the _n+m_-dimensional hyperplane of the convex hull of another matrix, B, that PSLQ tracks. The projection of the columns B onto the solution *cannot* contain a solution of <v, y<sub>ext</sub>> = 0 -- the key to how PSLQ works. The larger this projection is in every direction, the less likely it will be that PSLQ misses good solutions. So we want to make the smallest radius of this projection as large as possible with each row swap. Choosing a swap that maximizes this minimum radius, rather than the proxy for it in the diagonal of H, might improve the output of PSLQ. There would be a cost to pay in run time.
