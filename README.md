...
- _y<sub>ext</sub>_ not even being short! Though PSLQ is designed to produce short solutions, its intended use case is for non-integer inputs. When attacking LWE, inputs to PSLQ are integers, so there are many integer relations among them. PSLQ considers any of the plentiful integer solutions to be a win, and it terminates. There is a potential remedy to this problem. PSLQ makes a decision to swap a pair of rows of its internal matrix, H, in each iteration. The pair it chooses optimizes a function of the diagonal elements of H. But PSLQ also tracks an integer matrix, B, whose columns get close to the solution hyperplane. Small diagonal elements of H are a proxy for the size of the projection of B's columns onto the solution hyperplane. Instead of using that proxy, we could take a more direct approach to maximizing the projection. This would steadily sharpen the bound on the smallest solution of <_v_,_y<sub>ext</sub>_> = 0. Only when this bound can no longer be sharpened, would PSLQ be allowed to terminate. The implementation of PSLQ that the code in this repository uses is just whatever mpmath provides; it doesn't incorporate such a modification.
