...
The first constraint above may be difficult to pull off, and the two constraints are not a complete answer to lattice attacks. The attacker can still launch a statistical attack on biased plaintext, even without calculating _bit_ correctly every time, by finding a small enough _y_ -- albeit not as small as the private key, _x_ -- such that _Ay_ = _u_. With all that said, the easier constraint -- choosing _x_ so that some of the <_a_<sub>i</sub>, _x_> - _u_<sub>i</sub> are large multiples of _q_ -- is probably a good practice.
