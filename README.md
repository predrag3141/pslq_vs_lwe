...
So _v_ is an _n+m+1_-long concatenation of the _n_-long vector of the _base<sup>i</sup>q s_; an _m_-long combination of the _a<sub>i</sub> s_; and _u'_.

### Calling PSLQ

PSLQ *will* find a short solution _y<sub>ext</sub>_ of <_v_, _y<sub>ext</sub>_> = 0. (Subscript "ext" highlights the fact that _y<sub>ext</sub>_ is an extension of the deisred solution, _y_, of _Ay_ = _u_). If all goes well, _y_ = (_y<sub>ext n+1</sub>_, _y<sub>ext n+2</sub>_, ..., _y<sub>ext n+m</sub>_) will be a solution of _Ay_ = _u_ mod _q_.
